function obj = computeEAMSD(obj, indices)
%%COMPUTEEAMSD Compute the mean-squared-displacement for this object.
%
% obj = obj.computeEAMSD computes the MSD for all the tracks stored
% in this object. If a drift correction was computed prior to this
% method call, it is used to correct positions before MSD
% calculation.
%
% This version does not include time averaging (the MSD is only calculated
% between t = 0 and the specified time delay).
%
...
% N x 2, and is arranged as follow: [dt mean; ...] where dt
% is the delay for the MSD, and mean is the mean MSD value for this
%
% obj = obj.computeEAMSD(indices) computes the MSD only for the
...
fprintf('Computing (non time-averaged) MSD of %d tracks... ', n_tracks);
...
for i = 1 : n_tracks
    ...
    for j = 1 : 1 %changed from: for j = 1 : n_detections - 1
