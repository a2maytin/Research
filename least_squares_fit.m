...
r_expsim=ones(1,(sum(cellfun(@length, pos))-numel(pos))); %array will store experimental displacements
...
for i = 1:numel(pos) %loop through every track
    for j = 1:(length(pos{i})-1) %loop through every displacement
        ...
        r_expsim(k) = r_new*conversion;
        k=k+1;
    end
end

figure;
h1 = histogram(r_expsim)
...
title('SSD Histogram','FontSize', 14);

% Plot simulated "exp" trajectories
Dsim = 0.2;
steps = 10;
msteps = 50;
tausim = 0.02;
...
totR = params.totR;
r_expsim=ones(1,(size(rne,1)-1)*totR);  %array will store simulated displacements
...
hold on
h2 = histogram(r_expsim)
h2.Normalization = 'probability';
h2.BinWidth = 0.01;

% Plot simulated "theo" trajectories 
Dsim = 0.2;
steps = 10;
msteps = 50;
tausim = 0.02;
