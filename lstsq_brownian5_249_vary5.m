...
fbest=[0.074,0.279,0.481,0.158,0.008]; 

%let it go to 0? (z1) 1.0054
Dbest = [.0360 .0970 .2000 .6260 1.5230];
fbest = [.070 .299 .485 .146 0];
...
chisq_array = zeros([length(D1),length(D2),length(D3),length(D4),length(D5),length(f1),length(f2),length(f3),length(f5)]);
%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
tries = 1;
f4 = 1-f1(a)-f2(b)-f3(c)-f5(d);
fprintf('Fitting distribution for D=[%.3f,%.3f,%.3f,%.3f,%.3f], f=[%.3f,%.3f,%.3f,%.3f,%.3f]\n\n', ...
        D1(k),D2(l),D3(m),D4(n),D5(o),f1(a),f2(b),f3(c),f4,f5(d));

Dsim=[D1(k),D2(l),D3(m),D4(n),D5(o)];
fsim=[f1(a),f2(b),f3(c),f4,f5(d)];
